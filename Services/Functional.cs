...
namespace AMS.Services
{
    public class Functional : IFunctional
    {
        ...
        public async Task CreateAssetDistribution()
        {
            try
            {
                // Create Asset Assigned Data
                var _GetAssetAssignedList = _CommonData.GetAssetAssignedList();
                foreach (var item in _GetAssetAssignedList)
                {
                    item.CreatedDate = DateTime.Now;
                    item.ModifiedDate = DateTime.Now;
                    item.CreatedBy = "Admin";
                    item.ModifiedBy = "Admin";
                    _context.AssetAssigned.Add(item);
                    await _context.SaveChangesAsync();
                }
                
                // Create Asset History Data
                var _GetAssetHistoryList = _CommonData.GetAssetHistoryList();
                foreach (var item in _GetAssetHistoryList)
                {
                    item.CreatedDate = DateTime.Now;
                    item.ModifiedDate = DateTime.Now;
                    item.CreatedBy = "Admin";
                    item.ModifiedBy = "Admin";
                    _context.AssetHistory.Add(item);
                    await _context.SaveChangesAsync();
                }
            }
            catch (Exception ex)
        ...
        public async Task CreateDefaultOtherUser()
        {
            ...
            {
                item.RoleId = 1; // Super Admin role
                item.IsApprover = 1;
                ...
                var result = await _iAccount.CreateUserProfile(item, "Admin");
                var _ApplicationUser = result?.Item1;
                if (_ApplicationUser != null)
                {
                    await _userManager.AddToRoleAsync(_ApplicationUser, "Super Admin");
                }
            }
        }
