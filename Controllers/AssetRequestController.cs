...
namespace AMS.Controllers
{
    ...
    public class AssetRequestController : Controller
    {
        ...
        public AssetRequestController(ApplicationDbContext context, ICommon iCommon, INotificationService notificationService)
        {
            _context = context;
            _iCommon = iCommon;
        ...
        public async Task<IActionResult> AddEdit(AssetRequestCRUDViewModel vm)
        {
            try
            {
                ...
                else
                {
                    ...
                    try
                    {
                        var adminUsers = await _context.UserProfile
                            .Where(u => !u.Cancelled && u.RoleId == 1) // Admin role
                            .Select(u => u.Email)
                            .ToListAsync();

                        if (adminUsers.Any())
                        {
                            await _notificationService.SendAssetRequestNotification(vm, adminUsers.First());
                        }
                    }
                    catch (Exception ex)
                    {
