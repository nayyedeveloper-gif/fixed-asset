    ...
    try {
        ...
        $.ajax({
            ...
            error: function (xhr, status, error) {
                ...
                var errorMessage = "An error occurred while saving the asset.";
                if (xhr.responseText) {
                    try {
                        var errorObj = JSON.parse(xhr.responseText);
                        if (errorObj.AlertMessage) {
                            errorMessage = errorObj.AlertMessage;
                        }
                    } catch (e) {
        ...
        Swal.fire({
            title: "Error",
            text: "An error occurred while preparing the form data: " + error.message,
            icon: "error"
        });
    }
}
...
var PreparedFormObj = function () {
    ...
    if (_divQRCode && _divQRCode.children && _divQRCode.children.length > 0) {
        var _src = _divQRCode.children[0].toDataURL("image/png");
        _FormData.append('QRCodeImage', _src)
    } else {
        _FormData.append('QRCodeImage', '')
    }
